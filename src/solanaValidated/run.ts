...
const MAX_DUE_VALIDATIONS_PER_RUN = 10;
...
async function getJson(url:string){const r=await fetch(url,{headers:{accept:"application/json","user-agent":"radar-v24/2.4 validated"},signal:AbortSignal.timeout(GECKO_TIMEOUT_MS)});if(r.status===429)throw new GeckoRateLimitError();if(!r.ok)throw new Error(`GeckoTerminal ${r.status} for ${url}`);return r.json();}
...
async function cleanupDatabase(c:Client,now:number){
  const expired=await c.execute({sql:"DELETE FROM solana_validated_cases WHERE status='WAITING' AND pool_created_at IS NOT NULL AND pool_created_at < ?",args:[now-MAX_VALIDATION_AGE_MS]});
  const oldClosed=await c.execute("DELETE FROM solana_validated_cases WHERE status='CLOSED' AND id NOT IN (SELECT case_id FROM solana_validated_decisions)");
  const dueCutoff=now-MIN_VALIDATION_AGE_MS;
  const trim=await c.execute({sql:`DELETE FROM solana_validated_cases WHERE status='WAITING' AND pool_created_at <= ? AND id NOT IN (SELECT id FROM solana_validated_cases WHERE status='WAITING' AND pool_created_at <= ? ORDER BY entry_liquidity_usd DESC, COALESCE(entry_volume_h1_usd,0) DESC LIMIT ${MAX_DUE_BACKLOG})`,args:[dueCutoff,dueCutoff]});
  return{expiredWaitingDeleted:expired.rowsAffected,closedWithoutDecisionDeleted:oldClosed.rowsAffected,backlogDeleted:trim.rowsAffected};
}

async function collectWaiting(c:Client,now:number){const pools=(await discoverFresh()).filter(p=>now-p.createdAt>=0&&now-p.createdAt<MIN_VALIDATION_AGE_MS).sort((a,b)=>((b.liquidityUsd??0)-(a.liquidityUsd??0))||((b.volumeH1Usd??0)-(a.volumeH1Usd??0)));let added=0;for(const p of pools.slice(0,MAX_NEW_CANDIDATES_PER_RUN)){const r=await c.execute({sql:`INSERT INTO solana_validated_cases(pool_address,mint,symbol,pool_created_at,first_seen_at,entry_price,entry_liquidity_usd,entry_volume_h1_usd,entry_buys_h1,entry_sells_h1,status) VALUES(?,?,?,?,?,?,?,?,?,?, 'WAITING') ON CONFLICT DO NOTHING RETURNING id`,args:[p.poolAddress,p.mint,p.symbol,p.createdAt,now,p.price,p.liquidityUsd??0,p.volumeH1Usd,p.buysH1,p.sellsH1]});if(r.rows.length)added++;}return{discovered:pools.length,waitingAdded:added};}
...
async function listDueWaiting(c:Client,now:number){const r=await c.execute({sql:`SELECT * FROM solana_validated_cases WHERE status='WAITING' AND pool_created_at<=? AND pool_created_at>=? ORDER BY entry_liquidity_usd DESC, COALESCE(entry_volume_h1_usd,0) DESC LIMIT ${MAX_DUE_VALIDATIONS_PER_RUN}`,args:[now-MIN_VALIDATION_AGE_MS,now-MAX_VALIDATION_AGE_MS]});return r.rows.map(mapCase);}
async function listActive(c:Client){const r=await c.execute(`SELECT * FROM solana_validated_cases WHERE status='ACTIVE' ORDER BY first_seen_at ASC LIMIT ${MAX_ACTIVE_PER_RUN}`);return r.rows.map(mapCase);}
